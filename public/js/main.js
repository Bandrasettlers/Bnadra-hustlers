        ...
        // ============================================================
        // ATMOSPHERIC VIBES SELECTOR ENGINE
        // ============================================================
        function toggleVibeMenu(e) {
            if (e) e.stopPropagation();
            const widget = document.getElementById("vibeWidget");
            const dropdown = document.getElementById("vibeDropdown");
        ...
        function selectAmbientVibe(vibeKey, vibeName, emoji, e) {
            if (e) e.stopPropagation();
            const rootContainer = document.getElementById("app-root");
            if (!rootContainer) return;

            // Remove all other vibe classes
            rootContainer.classList.remove("vibe-sunset", "vibe-cyber", "vibe-mist", "vibe-breeze");
            
            // Add current vibe class
            rootContainer.classList.add(`vibe-${vibeKey}`);

            // Update header / widget text
            const textEl = document.getElementById("activeVibeName");
            if (textEl) textEl.innerText = vibeName;

            // Update active option styling
            const options = document.querySelectorAll(".vibe-option");
            options.forEach(opt => {
                if (opt.getAttribute("onclick") && opt.getAttribute("onclick").includes(`'${vibeKey}'`)) {
            ...
            localStorage.setItem("bandra_hustlers_active_vibe", vibeKey);
            ...
            // Play ambient tone / beep
            if (typeof playBeepSound === "function") {
                const pitchMap = {
                    'sunset': 329.63,  // E4 (warm, grounding)
                    'cyber': 523.25,   // C5 (high energy, tech)
                    'mist': 261.63,    // C4 (deep, calming green)
                    'breeze': 440.00   // A4 (airy, seaside)
                };
                playBeepSound(pitchMap[vibeKey] || 440, 0.15);
            }
        }
        ...
        document.addEventListener("click", () => {
            const widget = document.getElementById("vibeWidget");
            const dropdown = document.getElementById("vibeDropdown");
            if (widget && dropdown && widget.classList.contains("open")) {
                widget.classList.remove("open");
                dropdown.style.display = "none";
            }
        });

        // Initialize vibe on load
        function initAmbientVibe() {
            const savedVibe = localStorage.getItem("bandra_hustlers_active_vibe") || "sunset";
            const vibeNames = {
                'sunset': 'SUNSET HAZE',
                'cyber': 'CYBER NEON',
                'mist': 'PALI HILL MIST',
                'breeze': 'CARTER ROAD BREEZE'
            };
            const emojis = {
                'sunset': '🔮',
                'cyber': '⚡',
            ...
            selectAmbientVibe(savedVibe, vibeNames[savedVibe], emojis[savedVibe], null);
        }

        // Export to window
        window.toggleVibeMenu = toggleVibeMenu;
        window.selectAmbientVibe = selectAmbientVibe;
        ...
        window.addEventListener("DOMContentLoaded", () => {
            ...
            initAmbientVibe(); // Initialize the active aesthetic atmosphere vibe!
